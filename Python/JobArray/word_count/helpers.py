...
def cleantext(text):
    # Get text of file
    idx1 = [line.find("***") for line in text].index(0)
    idx2 = [line.find("***") for line in text[idx1+1:]].index(0)
    text = text[idx1+1:idx1+idx2]
    
    # Remove punctuation and extra whitespace
    text = " ".join(filter(lambda line : line != '\r\n', text))
    text = re.sub('\s+', ' ', re.sub(r'[^\w\s]', ' ', text)).lower()
...
def countwords(text):
    textSep = text.split(" ")[0:-1] #empty string at the end
    wordcounts = dict()
    for w in textSep:
      wordcounts[w] = wordcounts.get(w, 0) + 1.0
    return wordcounts
